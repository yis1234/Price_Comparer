""" Explains to the user what the numbers at the beginning of the feedback is
and the feedback at the end of the program becomes much more neater and
...
def budget_checker(budget, item_price, item_name, price_per_unit):
    ...
    if len(unit_list) == 0:
        ...
        print(f"\nAverage price per unit is ${np.mean(unit_list):.2f}")
    ...
    print("\nCheapest item is", item_name[item_price.index(min(item_price))])
    print("Most expensive item is", item_name[item_price.index(max(item_price))])
    print("\nCheapest item per unit is", item_name[price_per_unit.index(min(price_per_unit))])
    print("Most expensive item per unit is", item_name[price_per_unit.index(max(price_per_unit))])
    if len(unit_list) > 0:
        print(f"\nRecommended item is {item_name[price_per_unit.index(min(unit_list))]}")
